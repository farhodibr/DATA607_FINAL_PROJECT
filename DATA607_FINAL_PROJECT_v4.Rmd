...
We now we have a tidy song dataframe for each artist.
...
word_freq_decade_years <- word_freq_decade_years |>
  ...
  filter(!(word %in% c("it’s", "da", "dah", "duh", "ba", "shoo", "yeah", "ooh", "oh", "nuh", "i’m", "gonna", "bop", "you’re", "contributors", "lyrics", "la", "wanna"))) %>% # don't include these words in the analysis
...
We can see here that love is the most frequent word, and is the only word that has a positive or negative emotion attached to it. Possibly the most interesting takeaway from this graph is that grandma is in top words.
...
# set.seed(146)
# samples_billboard_1960_2016 <- wiki_hot_100s |>
...
song_jobs <- samples_billboard_1960_2016 |>
  distinct(artist, title, year, .keep_all = TRUE)

head(song_jobs)
```

This dataframe is tidy since each observation represents a single song on the Billboard Top 100. Each cell contains a single value as well.
...
We now have a tidy data frame where each observation represent a song, and it's top emotion.
...
songs_with_emotions_grouped_by_decade |>
  ...
  labs(title = "Emotion Trends in Billboard Lyrics by Decade", x = "Decade", y = "Count")
```

Looking at the count bar graph, Joy is still by far the most common top emotion, appearing in nearly 25 songs. Yet, the count gets much small after the 90s. This makes sense because the data we have access to only goes up to 2016, so the 2010 would have less data. For the 2000s decade, it's possible for some reason we could not find the lyrics to the songs, or a top emotion could not be specified. Let's check where the missing emotions are:
...
As you can see, the decade with the most missing emotions is the 2000s decade. If we were doing further analysis with more time, we could pinpoint exactly why this data is missing.
...
songs_with_emotions_grouped_by_decade_pct |>
  ...
  geom_col(position = "dodge") +
  labs(title = "Emotion Trends in Billboard Lyrics by Decade", x = "Decade", y = "Percentage")
```
...
songs_with_emotions_polarity |>
  ...
  geom_line(size = 1.2) +
  labs(title = "Positive vs. Negative Emotional Trends Over Time",
       x = "Year", y = "Proportion of Songs") +
  theme_minimal(base_size = 14)

```

Overall there's no major trend shown in this graph, but we can see in the 2000s positive emotions decreased, while negative emotions increased surpassing positive. This corresponds with our bar graph data.
