...
```{r}
# Create a function to perform sentiment analysis on a single song
get_song_sentiment <- function(artist_name, song_title) {
  ...
  res <- GET(
    url = base_url,
    add_headers(Authorization = paste("Bearer", genius_token)),
    query = query
  )
# If the API call fails, return NA 
  ...
  # Extract the list of potential matches from the API response
  hits <- fromJSON(content(res, "text", encoding = "UTF-8"), flatten = TRUE)$response$hits
  song_url <- NA
  for (i in seq_len(nrow(hits))) {
    if (str_detect(tolower(hits$result.primary_artist.name[i]), tolower(artist_name))) {
      song_url <- hits$result.url[i]
      break
    }
  }
  if (is.na(song_url)) {
  ...
  # Scrape lyrics
  page_html <- tryCatch(read_html(song_url), error = function(e) return(NULL))
  if (is.null(page_html)) {
    return(tibble
           (artist = artist_name, 
             song = song_title, 
  ...
  lyrics_nodes <- html_elements(page_html, "div[data-lyrics-container='true']")
  if (length(lyrics_nodes) == 0) return(tibble(artist = artist_name, song = song_title, positive = NA, negative = NA, score = NA, top_emotion = NA))

  lyrics_text <- lyrics_nodes %>%
    html_text2() %>%
    str_split("\n") %>%
    unlist() %>%
    str_remove_all("\\[[^\\]]+\\]") %>%
  ...
  lyrics_df <- tibble(line = seq_along(lyrics_text), text = lyrics_text)
  ...
  # Use NRC lexicon to get the most frequent emotion (excluding "positive"/"negative" - too vague)
top_emotion <- words %>%
...
# If no emotion found, set to NA
if (length(top_emotion) == 0) top_emotion <- NA

# Return artist, song, and top emotion
return(tibble(
  artist = artist_name,
  song = song_title,
...
```{r}
all_artists_songs <- read_csv("https://raw.githubusercontent.com/aaliyahmjh/DATA607Project/refs/heads/main/all_artists_songs.csv", show_col_types = FALSE) # few bug fixes needed, will need to reenter.

# Grab 5 songs from each of the top 10 artists
get_top5_valid_sentiments <- function(artist_name, song_titles, max_songs = 5) {
  results <- list()
  for (song in song_titles) {
    result <- get_song_sentiment(artist_name, song)

    if (!is.na(result$top_emotion)) {
      results <- append(results, list(result))
    }
  # Stop once we have enough(5) valid songs
    if (length(results) >= max_songs) {
      break
    }
  }

  # Combine all valid results into a dataframe
  if (length(results) > 0) {
    return(bind_rows(results))
  } else {
    return(tibble(artist = artist_name, song = NA, top_emotion = NA))
  }
}

artist_song_list <- all_artists_songs %>%
  group_by(artist) %>%
  arrange(title) %>%  # Optional: sort by title or year
  summarise(songs = list(title), .groups = "drop")

# Now apply get_first_valid_sentiment to each row
