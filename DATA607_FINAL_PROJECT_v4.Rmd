...
# Sampled Data
sample_songs_with_emotions_grouped_by_decade <- songs_with_emotions_sample |>
  filter(!is.na(top_emotion)) |>
  mutate(year = as.numeric(year)) |>
  group_by(decade = floor(year / 10) * 10, top_emotion)|>
  summarise(n = n(), .groups = "drop")
...
sample_songs_with_emotions_grouped_by_decade_pct |>
  ggplot(aes(x = factor(decade), y = percent, fill = top_emotion)) +
  geom_col(position = "dodge") +
  labs(title = "Emotion Trends in Sampled Billboard Song Lyrics by Decade", x = "Decade", y = "Percentage")
```
