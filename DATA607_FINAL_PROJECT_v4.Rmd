...
```{r}
#all_artists_songs <- all_artists_songs|>
#  filter(# few bug fixes needed, will need to reenter.
```
...
artist_song_list <- all_artists_songs %>%
  filter(!is.na(artist), artist != "artist") %>% # TEMP CODE -- TO BE REMOVED
  group_by(artist) %>%
  summarise(songs = list(title), .groups = "drop")

song_list_by_year <- all_artists_songs %>%
  filter(!is.na(artist), artist != "artist", year != "Source:[42]") %>% # TEMP CODE -- TO BE REMOVED
...
#Farhod changed valid_sentiments to speed it up with foreach
...
#   slice_head(n = 1)
#Farhod changed valid_sentiments_by_decade with foreach to speed it up
